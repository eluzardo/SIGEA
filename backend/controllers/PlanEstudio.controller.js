...
const agregarPlanEstudio = async (req,res) =>{
...
const mostrarPlanEstudio = async (req,res) =>{
    const conexion = await conexionSQL();
    const ps= await conexion
                    .request()
                    .query("select * from carreraTMP")       
    conexion.close();
    res.json(ps.recordset)
}

const modificarPlanEstudio = async (req,res) =>{
    ...
    }else if(pPlanEstudio != null && contadorParametros<1) query = query + "pPlanEstudio=@pPlanEstudio"
